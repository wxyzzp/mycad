...
class ANTLRTokenPtr {
public:
    ANTLRTokenPtr(ANTLRAbstractToken *addr=0){ptr_ = addr; ref();}
    ANTLRTokenPtr(const ANTLRTokenPtr &rhs)	{ptr_ = rhs.ptr_; rhs.ref();}
    ~ANTLRTokenPtr();

    // use ANTLRTokenPtr as a pointer to ANTLRToken
    ANTLRAbstractToken *operator-> () { return ptr_; }
    void operator = (const ANTLRTokenPtr& rhs);
    ...
    // JEL added const access to token pointer
    const ANTLRAbstractToken *operator-> () const { return ptr_; }
