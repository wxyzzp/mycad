...
ANTLRTokenBuffer(ANTLRTokenStream *_input, int _k, int _cs)
{
	...
	buffer = new _ANTLRTokenPtr[chunk_size+1];
	memset(buffer,0,sizeof(_ANTLRTokenPtr )*(chunk_size+1));
	if ( buffer == NULL ) {
...
//			z->deref();
	...
	if ( buffer!=NULL ) delete [] (buffer-1);
}
...
extendBuffer()
{
	...
	_ANTLRTokenPtr *tempBuffer;
	tempBuffer = new _ANTLRTokenPtr[buffer_size + 1];
	memcpy(tempBuffer, buffer-1, (buffer_size - chunk_size)* sizeof(_ANTLRTokenPtr));
	delete [] (buffer - 1);
	buffer = tempBuffer;
