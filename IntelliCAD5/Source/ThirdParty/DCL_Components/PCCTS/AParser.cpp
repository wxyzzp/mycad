...
ANTLRParser(ANTLRTokenBuffer *_inputTokens,
	...
	try
		...
		{
		TRACE("token_type : Allocation fails\n");
		return;
		}
	lap = 0;
	labase = 0;
...
void ANTLRParser::init(CString &wszFileName)
	{
...
LT(int i)
	{
		...
		{
		char buf[500];
		fprintf(stderr, "LT(%d): bufferSize=%d, minTokens=%d, LLk=%d\n",  
			  i, inputTokens->bufferSize(), inputTokens->minTokens(), LLk 
				);
		fflush(stderr);
		sprintf(
				 buf, 
...
syn(_ANTLRTokenPtr tok, ANTLRChar *egroup, SetWordType *eset,
	...
	CString wszFinalErrorString;
	static BOOL fInvalidErrorFlag = FALSE;

	// dev - 
	// Load the string for synatx errors and call LogErrors  method.
	wszResourceString.LoadString(IDS_SYNTAX_ERROR);
	// Set the error flag, to stop parsing and further tree generation.
	g_Main.m_DCLParser.set_ErrorOccured(TRUE);

	CString wszLookAheadString;
	wszLookAheadString = LT(1)->getText();
    
	// If invalid string is found.
	if ( wszLookAheadString[0] > 0)
		{
		wszLookAheadString.Replace(-1,' '); 
        wszLookAheadString.Remove(_T(' ')); 
		wszErrorString.Format(wszResourceString,
					m_wszFileName,line,wszLookAheadString);
		wszFinalErrorString += wszErrorString;
//		g_Main.m_DCLParser.LogErrors(wszErrorString); 
        fInvalidErrorFlag = FALSE; 
		}
	else
		{
		 // If invalid end of file.
		if (!fInvalidErrorFlag)
			{
			wszResourceString.LoadString(IDS_INVALID_END_OF_FILE);
			wszErrorString.Format(wszResourceString,
									 m_wszFileName,line);
//			g_Main.m_DCLParser.LogErrors(wszErrorString); 
			wszFinalErrorString += wszErrorString;
			fInvalidErrorFlag = TRUE;
			g_Main.m_DCLParser.DisplayError(wszFinalErrorString);  
			}
	...
	if ( !etok && !eset ) 
		{
		// Write a new line character to the error.log
		wszErrorString.Format("\n");
//		g_Main.m_DCLParser.LogErrors(wszErrorString); 
        wszFinalErrorString += wszErrorString;
		g_Main.m_DCLParser.DisplayError(wszFinalErrorString);  
		return;
	...
	if ( k==1 )
		{
		// This condition is met when a keyword is missing at the next token.
        wszResourceString.LoadString(IDS_MISSING_KEYWORD);
		wszErrorString.Format(wszResourceString);
//		g_Main.m_DCLParser.LogErrors(wszErrorString); 
		}
	else
	...
	if ( set_deg(eset)>0 ) 
		...
		wszErrorString.Format(wszResourceString);
        wszFinalErrorString += wszErrorString;
		wszErrorString.Format("%s", token_tbl[etok]);
        // If following tokens are missing they are formatted before they are written to the error.log.
		if ( wszErrorString == "COLON")
			{
			wszErrorString = _T(" :") ;
			}
		if ( wszErrorString == "SEMI" )
			{
			wszErrorString = _T(" ;") ;
			}     
		
		if ( wszErrorString == "EQUALS" )   
			{
			wszErrorString = _T(" =") ; 
			}
		
		if ( wszErrorString == "LCURLY" )   
			{
			wszErrorString = _T(" {") ;
			}

		if ( wszErrorString == "RCURLY" )
			{
			wszErrorString = _T(" }") ;
			}
			
		if ( wszErrorString == "ATRATEOF" ) 
			{
			wszErrorString = _T(" @") ;
			}

		if ( wszErrorString == "INCLUDE")
			{
			wszErrorString = _T(" include") ;
			}
		
        wszErrorString = "\"" + wszErrorString + "\"";		



		if ( wszErrorString != _T(" Invalid") )
			{
//				g_Main.m_DCLParser.LogErrors(wszErrorString); 
				wszFinalErrorString += wszErrorString;
	...
	if ( strlen(egroup) > 0 ) 
		{
		// This condition has not yet occured, during trail and errors...
		wszErrorString.Format(" in?????? %s", egroup);
		g_Main.m_DCLParser.LogErrors(wszErrorString); 
		}

	if ( wszFinalErrorString.GetLength() != 0)
		{
		g_Main.m_DCLParser.DisplayError(wszFinalErrorString);  
		}

		ThrowCExceptionManager(ERROR_IN_DCL_FILE);
	}
...
edecode(SetWordType *a)
	...
	CString wszErrorString;
	wszErrorString.Format(" %s, ",token_tbl[e]);
	
	// Write to error.log if keywords are missing and this condition is called.
	if ( wszErrorString != _T("intlit") )
		{
		wszErrorString.LoadString (IDS_KEYWORD) ;
//		g_Main.m_DCLParser.LogErrors(wszErrorString);
		}
	else
		{
		 // If key attributes are missing, they display this string. 
		 wszErrorString.LoadString(IDS_ATTRIBUTE_VALUE_REQUIRED); 
//		 g_Main.m_DCLParser.LogErrors(wszErrorString);
		 return;
		}

    // Read value from token table, and write to error.log.    
	wszErrorString.Format(" %s, ",token_tbl[e]);
	if ( wszErrorString != _T(" Invalid, ") &&  wszErrorString != _T(" @, ") )
		{
//	  		g_Main.m_DCLParser.LogErrors(wszErrorString); 
		}
	...
	do 
		...
		do 
			...
			if ( t & *b )
				{
				wszErrorString.Format(" %s, ",token_tbl[e]);
				wszErrorString.MakeLower(); 
				if ( wszErrorString == _T(" intlit, ") )
					{
					// If an attribute is missing , then display this string.
					wszErrorString.LoadString(IDS_ATTRIBUTE_VALUE_REQUIRED);
//					g_Main.m_DCLParser.LogErrors(wszErrorString);  
					return;
					}
				// If tiles are required then get into this condition, and display this string. 
				if ( wszErrorString != _T(" invalid, ") &&  wszErrorString != _T(" @, ") )
					{
//					g_Main.m_DCLParser.LogErrors(wszErrorString); 
					iCounter++;
			...
			e++;
			} while (++b < &(bitmask[sizeof(SetWordType)*8]) && iCounter<3 );
		} while (++p < endp);
	if ( set_deg(a)>1 ) 
		{
		wszErrorString.Format(_T("...}"));
...
/*  fprintf(stdout, "%s not in %dth set\n", zztokens[LA(i)], i);*/
    *miss_set = f[i-1];
    *miss_text = text;
    *bad_tok = LT(i);
    *bad_text = (*bad_tok)->getText();
...
_setmatch_wdfltsig(SetWordType *tokensWanted,
	...
	if ( !set_el(LA(1), tokensWanted) )
		{
		TRACE("The _setmatch_wdfltsig called-during error generation");
		fprintf(ErrOutput,
				"line %d: syntax error at \"%s\" missing %s\n",
				LT(1)->getLine(),
				(LA(1)==eofToken)?"<eof>":LT(1)->getText(),
...
panic(char *msg)
	...
	fprintf(ErrOutput, "ANTLR panic: %s\n", msg);
	fclose(ErrOutput);
	exit(EXIT_FAILURE);
