...
errstd( const char *s)
	...
	// -Gurudev
    // This function occurs wehnan Invalid token is found.
	// When this function is called, then an error is written to the
	// Error.log, and the m_bErrorOccurred flag is set to TRUE.
	CString wszErrorString,wszResourceString,wszFinalErrorString;

	wszResourceString.LoadString(IDS_FILE_NAME);
	wszErrorString.Format(wszResourceString,
						  this->input->m_wszFileName);	
//	g_Main.m_DCLParser.LogErrors(wszErrorString);  
    wszFinalErrorString = wszErrorString;
    g_Main.m_DCLParser.set_ErrorOccured(TRUE); 

	TRACE("%s",this->input->m_wszFileName);


	wszResourceString.LoadString(IDS_INVALID_CHAR_NEAR_LINE);
	wszErrorString.Format(wszResourceString,
			_line,((s == NULL) ? "Lexical error" : s)
            ,_lextext);
//	g_Main.m_DCLParser.LogErrors(wszErrorString);  
	wszFinalErrorString += wszErrorString;
...
erraction()
{
	...
	// errstd("invalid token"); - this statement is replaced by the following to support localization
    //
    CString wszErrorString;
	wszErrorString.LoadString(IDS_INVALID_TOKEN);
    errstd((LPCTSTR)wszErrorString);
