	...
	virtual void tracein(char *r)
			{
				...
				fprintf(ErrOutput, "enter rule \"%s\"\n", r);
				fclose(ErrOutput);
			}
...
public:
	...
	virtual void init(CString &wszFileName);
...
#define zzfailed_pred(_p)       \
  zzNON_GUESS_MODE { FILE *ErrOutput; ErrOutput = fopen(ERROR_LOG,"a+"); fprintf(ErrOutput, "line %d: semantic error; failed predicate: '%s'\n",LT(1)->getLine(), _p); fclose(ErrOutput);}
#endif
