...
GetUpperDimgap()
{
  ...
  double d = plinkvar[DIMGAP].dval.rval * dDimscale;
  if (d < 0)
    {
      d *= 2;
    }
  d  = fabs(d);
  d += plinkvar[DIMTXT].dval.rval * dDimscale / 2;
  return d;
}
...
{
  dl.showArw1 = FALSE;
  if (plinkvar[DIMTOFL].dval.ival)
    {
      // draw center line
      dl.showTofl = TRUE;
      ic_ptcpy(dl.toflS, plink->pt15);

      if (bIsDia)
        {
          // if diametric

          // center line is from defpt to pt15 (from arrow to arrow)
          double ang = sds_angle(plink->vertex, plink->pt15);
          ang += IC_PI;
          double d = sds_distance(plink->vertex, plink->pt15);
          sds_polar(plink->vertex, ang, d, dl.toflE);
          ...
          if (plinkvar[DIMSD1].dval.ival && plinkvar[DIMSD2].dval.ival)
          ...
          else
            {
              ...
              sds_polar(plink->vertex, ang, d + dDimasz,     dl.extS);
              sds_polar(plink->vertex, ang, d + dDimasz * 2, dl.extE);
            }
        }
      else
        {
          ...
          // center line is from defpt to pt15 (from center to arrow)
          ic_ptcpy(dl.toflE, plink->vertex);
        }
    }
  else
    {
      // draw center mark
      cmd_CenterMark(plink->vertex,flp,dragmode);
    }
  return TRUE;
}
...
{
  sds_point ps, pe;
  double d = plink->distance;

  // start angle
  double dS = plink->sangle;
  ps[0] = d * cos(dS) + plink->vertex[0];
  ps[1] = d * sin(dS) + plink->vertex[1];
  ps[2] = plink->vertex[2];

  // end angle
  double dE = plink->eangle;
  pe[0] = d * cos(dE) + plink->vertex[0];
  pe[1] = d * sin(dE) + plink->vertex[1];
  pe[2] = plink->vertex[2];

  // get near edge point
  dS = sds_distance(plink->textmidpt, ps);
  dE = sds_distance(plink->textmidpt, pe);
...
)
{
  ...
  // EBATECH(CNBR) -[ DIMTMOVE==2 && DIMTEDIT /HOME
  if( dragmode == 0 && plink->HomeText == TRUE && plinkvar[DIMFIT].dval.ival == 5 )
	iDimfit = 3;
  // EBATECH(CNBR) ]-
  ...
  switch (iDimfit)
    {
      ...
      case 2:
          return cmd_DimRad0(
                    flp, txtbox, box2, dl, angle, dragmode, bIsDia);
...
)
{
    ...
    short          fi1 = 0,
        ...
        // Put the center of the circle or arc in defpt.
        ...
        if (plink->GenInitflg || SDS_AtNodeDrag) {
            ...
            if (plinkvar[DIMFIT].dval.ival==5) {
                ...
                // Calculate the insertion point for the text.
                sds_polar(plink->pt13,angle,(plink->distance/2),plink->textmidpt);
            }
        }
        // Copy the center point of the circle to ptDLine[0].
        ic_ptcpy(ptDLine[0],plink->defpt);
        // Calculate the point ((plinkvar[DIMASZ].dval.rval*plinkvar[DIMSCALE].dval.rval)) closer to the center from the radius of the circle in ptDLine[2].
        ...
        if (((angle>=plink->sangle || angle<=plink->eangle) || (!plink->sangle && !plink->eangle)) && (plinkvar[DIMFIT].dval.ival!=3 && plinkvar[DIMFIT].dval.ival!=4)) {
            ...
            if (fi2==3 || fi1==3) {
                ...
                // If the distance of the dimension line is less then the text (and/or) the arrows set TextFit to false.
                if (fr1>plink->dimvalue) {
                    switch(plinkvar[DIMFIT].dval.ival) {
                        case 0: ArrowFit=FALSE; TextFit=FALSE; break;
                        case 1: ArrowFit=FALSE; break;
                        case 2: TextFit=FALSE; break;
                    }
                }
            } else {
                TextFit=FALSE;
                switch(plinkvar[DIMFIT].dval.ival) {
                    ...
                    case 1: ArrowFit=FALSE; break;
                    case 2:
                    case 5: fr1=((plinkvar[DIMASZ].dval.rval*plinkvar[DIMSCALE].dval.rval)*2);
                            if (fr1>plink->dimvalue) ArrowFit=FALSE;
                            break;
                }
            }
            ...
            // If only the arrows fit.
            } else if (ArrowFit) {
                TextRotation=(!plink->txtangflag && !plinkvar[DIMTOH].dval.ival)?(angle):plink->textrot;
                // Creating the interior lines.
            ...
            // If the Text fits but the arrows don't.
                ...
                dimDim2flag=FALSE;
                cmd_CenterMark(plink->defpt,flp,dragmode);
            ...
            // If the arrows don't fit with the text then neither will fit.
                ...
                // If (DIMFIT==5) make sure that (plink->textmidpt) is not calcualted.
                if (plinkvar[DIMFIT].dval.ival!=5) {
                    // If DIMTOH==0 then make sure the text is at (angle)
                    if (!plinkvar[DIMTOH].dval.ival) {
                        dimDim2flag=FALSE;
                        // Moveing the text to the correct position.
        ...
        // If the angle is outside of the arc.
            ...
            if (plink->sangle) 
                ic_normang(&plink->sangle,&angle);
            if (angle>=plink->eangle && (plink->sangle || plink->eangle)) {
                sds_polar(plink->defpt,(angle+IC_PI),plink->distance,ptDLine[1]);
            } else {
                ic_ptcpy(ptDLine[1],plink->pt15);
                // Setup center mark points.
                // Calculate the center of the arc or circle and put it in ptt[0]
                cmd_CenterMark(plink->defpt,flp,dragmode);
            }
            dimDim2flag=FALSE;
            ...
            if (applyHozText) {
                TextRotation=(!plink->txtangflag)?0.0:plink->textrot;
                MakeExteraLine=TRUE;
...
//                    }
            ...
            // Make the text the same as (angle)
            } else {
