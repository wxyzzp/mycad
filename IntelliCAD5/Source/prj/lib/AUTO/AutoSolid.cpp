/* INTELLICAD\PRJ\LIB\AUTO\AUTOSOLID.CPP
 * Copyright (C) 1997-1999 Visio Corporation. All rights reserved.
 * 
 * Abstract
 * 
 * Implementation of CAutoSolid - the Automation Solid object
 * 
 */ 
...
HRESULT CAutoSolid::Init(CIcadDoc *pDoc, IDispatch *pParent, db_handitem *pDbHandItem)
	{
	...
	if (pDbHandItem)
		...
		{
		if (pDoc)
			m_pDbHandItem = new db_solid(pDoc->m_dbDrawing);
		else
			m_pDbHandItem = new db_solid();
		}
...
STDMETHODIMP CAutoSolid::get_Coordinates (IIcadPoints * * ppCoords)
	{
...
STDMETHODIMP CAutoSolid::put_Coordinates (IIcadPoints * ppCoords)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	CComPtr<IIcadPoint> pIIcadPoint = NULL;
	HRESULT hr;
...
STDMETHODIMP CAutoSolid::get_Normal (IIcadVector * * ppExtrusionDirection)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
...
STDMETHODIMP CAutoSolid::put_Normal (IIcadVector * ExtrusionDirection)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	sds_point v;
	v[0] = v[1] = v[2] = 1.0;
...
STDMETHODIMP CAutoSolid::get_Thickness (double * pThick)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	if (pThick == NULL)
		return E_POINTER;
...
STDMETHODIMP CAutoSolid::put_Thickness (double pThick)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
