/* D:\INTELLICAD\PRJ\LIB\AUTO\AUTOINFLINE.CPP
 * Copyright (C) 1997-1999 Visio Corporation. All rights reserved.
 * 
 * Abstract
 * 
 * Implementation of CAutoInfLine - the Automation Infinite Line object
 * 
 */ 
...
HRESULT CAutoInfLine::Init(CIcadDoc *pDoc, IDispatch *pParent, db_handitem *pDbHandItem)
	{
	...
	if (pDbHandItem)
		...
		{
		if (pDoc)
			m_pDbHandItem = new db_xline(pDoc->m_dbDrawing);
		else
			m_pDbHandItem = new db_xline();
		}
...
STDMETHODIMP CAutoInfLine::get_BasePoint (IIcadPoint * * ppBasePoint)
	{
...
STDMETHODIMP CAutoInfLine::put_BasePoint (IIcadPoint * ppBasePoint)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	sds_point pt;
	pt[0] = pt[1] = pt[2] = 0.0;
...
STDMETHODIMP CAutoInfLine::get_DirectionVector (IIcadVector * * ppDirection)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	if (ppDirection == NULL)
		return E_POINTER;
...
STDMETHODIMP CAutoInfLine::put_DirectionVector (IIcadVector * ppDirection)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
	sds_point pt;
...
STDMETHODIMP CAutoInfLine::Offset (double Offset, IIcadSelectionSet * * ppArrayOfEntities)
	{
	RETURN_IF_INVALID (m_pDbHandItem);
