...
/*F*/
db_ray::db_ray(void) : db_entity(DB_RAY) {

    pt[0]=pt[1]=pt[2]=vect[1]=vect[2]=0.0; vect[0]=1.0;
    disp=NULL; memset(extent,0,sizeof(extent));
}
db_ray::db_ray(db_drawing *dp) : db_entity(DB_RAY,dp) {

    pt[0]=pt[1]=pt[2]=vect[1]=vect[2]=0.0; vect[0]=1.0;
    disp=NULL; memset(extent,0,sizeof(extent));
}
db_ray::db_ray(const db_ray &sour) :
