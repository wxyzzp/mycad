...
/*F*/
db_arc::db_arc(void) : db_entity_with_extrusion(DB_ARC) {
    thick=cent[0]=cent[1]=cent[2]=ang[0]=0.0; rad=ang[1]=1.0;
    disp=NULL; memset(extent,0,sizeof(extent));
}
db_arc::db_arc(db_drawing *dp) : db_entity_with_extrusion(DB_ARC,dp) {
    char *bufp;

    bufp=(dp!=NULL) ? dp->ret_headerbuf() : NULL;

    db_qgetvar(DB_QTHICKNESS,bufp,&thick,DB_DOUBLE,0);
    cent[0]=cent[1]=cent[2]=ang[0]=0.0; rad=ang[1]=1.0;

    disp=NULL; memset(extent,0,sizeof(extent));
}
db_arc::db_arc(const db_arc &sour) :
    db_entity_with_extrusion(sour) {  /* Copy constructor */

    thick=sour.thick;

    DB_PTCPY(cent,sour.cent); rad=sour.rad;
    ang[0]=sour.ang[0]; ang[1]=sour.ang[1];
...
*/
    ...
    /* 51 (ang[1]) */
    if ((sublist[1]->rbnext=db_newrb(51,'R',ang+1))==NULL) { rc=-1; goto out; }
    sublist[1]=sublist[1]->rbnext;
