...
/*F*/
db_trace::db_trace(void) : db_entity_with_extrusion(DB_TRACE) {
...
db_trace::db_trace(db_drawing *dp) : db_entity_with_extrusion(DB_TRACE,dp) {
    char *bufp;
...
db_trace::db_trace(const db_trace &sour) :
...
db_trace::~db_trace(void) {

    if (db_grafreedofp!=NULL) db_grafreedofp(disp);
}

/*F*/
int db_trace::entgetspecific(
    struct resbuf **begpp,
    struct resbuf **endpp,
    db_drawing     *dp) {
/*
