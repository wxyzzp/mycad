...
/*||||||||||||||||||||||||||||||||||||||||
||		Class Structures & Functions	||
||		taken from "Implementing 		||
||		DDE Using C++ Classes" by		||
||		Nigel Thompson					|| 
||||||||||||||||||||||||||||||||||||||||*/
...
BOOL CDDEConv::ConnectTo(const char* pszService, const char* pszTopic)
{
    ...
    if (!m_hConv) {
        m_pServer->Status(ResourceString(IDC_ICADSTDDE_FAILED_TO_CONN_0, "Failed to connect to %s|%s. Error %u" ),
...
BOOL CDDEServerSystemTopic::Request(UINT wFmt, const char* pszItem,
                                    void** ppData, DWORD* pdwSize)
{
    m_pServer->Status(ResourceString(IDC_ICADSTDDE_SYSTEM_TOPIC_R_2, "System topic request: %s" ), pszItem);
                        ...
                        DWORD* pdwDDEInst/* = NULL */)
{
    ...
    if (pTheServer != NULL) {
        TRACE(ResourceString(IDC_ICADSTDDE_ALREADY_GOT_A__3, "Already got a server!\n" ));
        ASSERT(0);
        return FALSE;
    } else {
        pTheServer = this;
                                            ...
                                            DWORD dwData2)
{
    ...
    ASSERT(pServ);
    pServ->Status(ResourceString(IDC_ICADSTDDE_CALLBACK__4_4X_4, "Callback %4.4XH" ), wType);
    ...
    case XTYP_CONNECT_CONFIRM:
        ...
        //
        pServ->Status(ResourceString(IDC_ICADSTDDE_CONNECT_TO__S_5, "Connect to %s" ), (const char*)pServ->StringFromHsz(hsz1));
    ...
    case XTYP_DISCONNECT:
        ...
        pServ->Status(ResourceString(IDC_ICADSTDDE_DISCONNECT_6, "Disconnect" ));
    ...
    case XTYP_WILDCONNECT:
        ...
        || !::DdeCmpStringHandles(hsz2, pServ->m_hszServiceName)) {
            ...
            pServ->Status(ResourceString(IDC_ICADSTDDE_WILD_CONNECT_T_7, "Wild connect to %s" ), (const char*)pServ->StringFromHsz(hsz1));
...
HDDEDATA CDDEServer::DoWildConnect(HSZ hszTopic)
{
    ...
    int iTopics = 0;
    CString strTopic = ResourceString(IDC_ICADSTDDE__NULL__8, "<null>" );
    if (hszTopic == NULL) {
    ...
    if (!iTopics) {
        Status(ResourceString(IDC_ICADSTDDE_WILD_CONNECT_T_9, "Wild connect to %s refused" ), (const char*)strTopic);
        return (HDDEDATA) NULL;
    }
                            ...
                            HDDEDATA *phReturnData)
{
    ...
    if (wType == XTYP_EXECUTE) {
        ...
        Status(ResourceString(IDC_ICADSTDDE_EXEC_10, "Exec" ));
        DWORD dwLength = 0;
        ...
        Status(ResourceString(IDC_ICADSTDDE_EXEC_FAILED_11, "Exec failed" ));
    ...
    case XTYP_ADVSTART:
        ...
        if (!CanAdvise(wFmt, strTopic, strItem)) {
            ...
            Status(ResourceString(IDC_ICADSTDDE_CAN_T_ADVISE__12, "Can't advise on %s|%s" ), (const char*)strTopic, (const char*)strItem);
            return FALSE;
        }
    ...
    case XTYP_POKE:
        ...
        if (!b) {
            ...
            Status(ResourceString(IDC_ICADSTDDE_POKE__S__S_FA_13, "Poke %s|%s failed" ), (const char*)strTopic, (const char*)strItem); 
            return FALSE;
    ...
    case XTYP_ADVDATA:
        ...
        if (!b) {
            ...
            Status(ResourceString(IDC_ICADSTDDE_ADVISEDATA__S_14, "AdviseData %s|%s failed" ), (const char*)strTopic, (const char*)strItem); 
            *phReturnData = (HDDEDATA) DDE_FNOTPROCESSED;
    ...
    case XTYP_REQUEST:
        ...
        Status(ResourceString(IDC_ICADSTDDE_REQUEST__S__S_15, "Request %s|%s" ), (const char*)strTopic, (const char*)strItem); 
        dwLength = 0;
        if (!Request(wFmt, strTopic, strItem, &pData, &dwLength)) {
            ...
            Status(ResourceString(IDC_ICADSTDDE_REQUEST__S__S_16, "Request %s|%s failed" ), (const char*)strTopic, (const char*)strItem); 
            *phReturnData = NULL;
            return FALSE;
...
{
    CString str = ResourceString(IDC_ICADSTDDE__NULL__8, "<null>" );
    ...
    if (dw == 0) str = ResourceString(IDC_ICADSTDDE__ERROR__17, "<error>" );
...
{
    CString strName = ResourceString(IDC_ICADSTDDE__1, "" );
    PCFTAGNAME pCTN;
